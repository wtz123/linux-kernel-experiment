...
int myevent_open(int,int);
int myevent_wait(int);
int myevent_signal(int);
int myevnent_close(int);
//myevent_open(0,1);//不能在全局上调用函数？必须得放在方法里面？
...
//me modify
int getpfcount(long *pf,long *jiff)
{//获取缺页计数器的值

	long p=pfcount;
	long j=jiffies;
	if(copy_to_user(pf,&p,sizeof(p)))
	{return -EFAULT;}
	
	if(copy_to_user(jiff,&j,sizeof(j)))
	{return -EFAULT;}
	
	return 0;
}
EXPORT_SYMBOL(getpfcount);


/*原先自己写在sys.c中的，但是为了迎合实验六的b实验，把核心代码写入了fault.c，以便fault.c中的函数可以调用！！！！！！！！！！！*/
//内核同步机制
typedef struct __myevent
{
	int eventNum;// 事件号
	wait_queue_head_t p; // 系统等待队列首指针
	struct __myevent *next; // 事件队列链指针
	int count;//模拟信号量
	int init_count;//用来控制sianal时，不超过初始值
}myevent_t;

myevent_t * lpmyevent_head = NULL ; // 事件链头指针
myevent_t * lpmyevent_end = NULL ; // 事件链尾指针
spinlock_t mr_lock=SPIN_LOCK_UNLOCKED;//互斥锁	用于对count的操作
...
int myevent_open(int eventNum,int c)
/*
* me modify 对原先的函数进行了修改
当eventNum代表的事件不存在时，用传入的时间号作为新建的事件号
当存在时，直接返回，没有动作
*/
{
	myevent_t *new;
	...
	if(eventNum)
	{ 
	if(scheventNum(eventNum, &prev))//说明该事件存在
		return 0;
	//事件不存在。
...
spin_lock(&mr_lock);//对count的操作要互斥进行
tmp->count--;
spin_unlock(&mr_lock);
if(tmp->count<0)
	{
...
if((tmp = scheventNum(eventNum,&prev)) != NULL) {
spin_lock(&mr_lock);
if((tmp->count+1)<=tmp->init_count)//信号量的count不能加到比初始值还大！
tmp->count++;
spin_unlock(&mr_lock);
	if(tmp->count<=0)
	{	//说明等待队列中还有进程
		wake_up(&tmp->p);//唤醒等待队列中的一个互斥进程
		return eventNum;
	}
}
return 0;
}
EXPORT_SYMBOL(myevent_signal);
...
myevent_signal(eventNum); //如果阻塞则唤醒它
kfree(releaseItem); //释放事件节点
return eventNum;
}
return 0;
}
EXPORT_SYMBOL(myevent_close);
